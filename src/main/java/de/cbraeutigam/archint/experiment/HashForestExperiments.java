...
public class HashForestExperiments {
	...
	private static String[] fileNames = new String[] { "00", "01", "02", "03",
...
//		}
		String integrityFile1 = "testdata/integrityfiletest";
//		BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(
//				new FileOutputStream(integrityFile1), Charset.forName("UTF-8")));
//		hf1.pruneForest();
//		hf1.writeTo(bw);
//		bw.close();
		
		HashForest<SHA512HashValue> hf2 = new HashForest<SHA512HashValue>();
		hf2.readFrom(new InputStreamReader(
				new FileInputStream(integrityFile1), Charset.forName("UTF-8")));
		System.out.println(hf2);
		
//		System.out.println(hf1.validate(hf2));
		
		HashForest<SHA512HashValue> hf3 = new HashForest<SHA512HashValue>();
		for (String hashString : hashStrings) {
			hf3.update(new SHA512HashValue(hashString));
		}
		for (String hashString : hashStrings) {
			hf3.update(new SHA512HashValue(hashString));
		}
		
		System.out.println(hf3.validate(hf2));
		System.out.println(hf3.contains(hf2));
