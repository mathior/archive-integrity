...
public class DemoApplication {
	...
	private class CliOptions {
		...
		public CliOptions(Option option, boolean full, String path1, String path2) {
			this.option = option;
			if (full) {
				mode = Mode.FULL;
			} else {
	...
	public static String createIntegrityData(String baseDir, HashForest.Mode mode)
		...
		/*
		 * ordering should use relative paths, therefore use the full path and
		 * cut the full path part of the base dir
		 */
		File baseDirF = new File(baseDir);
		for (File f : fileTree) {
			ordering.add(f.getAbsolutePath().substring(baseDirF.getAbsolutePath().length() + 1));
		}
		...
		statusMessage.append(integrityFile.getAbsolutePath());
		statusMessage.append("\n");
		statusMessage.append("Ordering information written to: ");
		statusMessage.append(orderFile.getAbsolutePath());
		statusMessage.append("\n");
	...
	private static void printUsageAndExit(int exitStatus, String errorMessage) {
		String usageMessage = new String("Usage: java -jar demo.jar [OPTIONS] PATHS\n"
...
//		args = new String[] {"-c", "--full", testDirCurrent};
		...
		if (opts.getOption().equals(Option.HELP)) {
			...
			System.out.println(createIntegrityData(opts.getPath1(), opts.getMode()));
		} else if (opts.getOption().equals(Option.CHECK)) {
			if (checkIntegrity(opts.getPath1())) {
				System.out.println("VALID");
			} else {
				System.out.println("INVALID");
			}
		} else if (opts.getOption().equals(Option.CONTAINS)) {
			if (checkContains(opts.getPath1(), opts.getPath2())) {
				System.out.println("VALID");
